...
AI-powered job search pipeline: scan portals, evaluate offers against your resume, generate tailored PDFs, track applications. Agent-driven — no server, no database, everything is files. Differentiator: the skills progression loop (gap analysis, resume polish, progression tracking).
...
node scripts/generate-feeds.mjs              # Generate local Indeed + LinkedIn feed files (skip if <24h old)
...
node scripts/generate-feeds.mjs --auth linkedin  # Interactive login — saves LinkedIn session for full results
```
...
- `resume.md` — canonical CV (copy from `resume.example.md`, gitignored). Source of truth for bullet bank generation and tailoring. May have slightly different bullet text than the master HTML — always read actual content before writing patches; don't assume it matches the HTML word-for-word.
- `output/resume-master-{date}.html` — polished master resume HTML, manually maintained. Find the current one with `ls output/resume-master-*.html`. This file accumulates edits over time and may diverge from `resume.md`. When making global edits (verb fixes, compression, writing quality), apply to both this file and `resume.md`, then regenerate its PDF: `node scripts/generate-pdf.mjs output/resume-master-{date}.html output/resume-master-{date}.pdf`. The master HTML and `resume.md` are sister documents — never update one without updating the other.
- `career-bank.md` — bullet bank, summary paragraphs, and keyword frequency tracker for tailored resume generation (gitignored). Structure: numbered Summary paragraphs (1–5, role-type keyed), per-role bullet sections with `<!-- tags: keyword1, keyword2 -->` comments for keyword matching, and a Keyword Frequency Tracker table (`Keyword | Count | Last Seen`). Summaries are indexed by role type: 1=Manager+Security, 2=Manager+Platform/Product, 3=Director/VP, 4=IC/Staff, 5=AI/ML. Bullet text in career-bank often differs from resume.md — always read before patching.
- `training/skills.md` — learn-to-qualify pipeline: skills to study, prioritized by JD frequency (gitignored). Columns: `Skill | JD Count | Resource | Est. Time | Status | Started | Completed`. Status values: `not started`, `in progress`, `done`. Priority derived at display time from JD Count: High ≥ 6, Medium 3–5, Low < 3.
...
- `data/company-info.json` — persistent cache of company headquarters data (address, city, state, zip, phone, website) used by `scripts/generate-log.mjs` to populate TWC work search log employer fields. Add entries here for any new company applied to. Gitignored.
- `data/employer-urls.json` — manual override map of company name → career URL for companies not auto-derived from portals.yml (e.g. Apple, Dragos, The Athletic). Add entries here when applying to a company not in portals.yml. generate-log uses these URLs for the "Person Contacted" field. Gitignored.
- `data/feeds/` — local job feed files generated by `scripts/generate-feeds.mjs`. `data/feeds/indeed.json` and `data/feeds/linkedin.json` are consumed by the `localfeed` adapter in `scan.mjs`. Auth sessions stored in `data/feeds/.auth/` (e.g. `linkedin-state.json`). Entire directory is gitignored.
...
**Text alignment rule:** All tailored resume and cover letter HTML files must use justified body text. For resumes: `p { text-align: justify; }` and `li { text-align: justify; }`. For cover letters: `p { text-align: justify; }`. Apply this to every generated HTML file without exception.
...
- Every lead verb must be unique across the entire resume — no verb may open more than one bullet anywhere in the document. Assign verbs before drafting; don't discover duplicates after. After any global edit, run a verb audit: `node -e "const r=require('fs').readFileSync('resume.md','utf8');const b=r.split('\n').filter(l=>l.trim().startsWith('- '));const v=b.map(l=>{const m=l.trim().replace(/^- /,'').match(/^([A-Z][a-z]+)/);return m?m[1]:null}).filter(Boolean);const c={};v.forEach(x=>c[x]=(c[x]||0)+1);const d=Object.entries(c).filter(([,n])=>n>1);console.log(d.length?'DUPES: '+JSON.stringify(d):'All unique.');"` (or use `import` syntax in an `.mjs` file)
...
- Confident, short close: use an affirmative, forward-looking opener ("Looking forward to connecting.") followed by a brief sign-off thank-you. No more than three sentences total. End with a name-only sign-off ("{{YOUR_NAME}}"). Acceptable closing thank-yous: "Thanks for your time.", "Thanks for reading.", "Appreciate the time." — short and casual, not ceremonial.
...
- Every cover letter HTML file must include a contact header immediately after `<body>`: `<div class="contact">{{NAME}} · {{EMAIL}} · {{PHONE}}</div>` with `.contact { font-size: 9pt; color: #555; margin-bottom: 16px; }` in the style block.
- Every cover letter HTML file must use justified body text: `p { text-align: justify; }` in the style block.
...
6. **iCIMS** — two patterns: `careers_host` (Phenom/careers.X.com API, supports optional `categories` filter) or `slug`-only fallback to `{slug}.icims.com/jobs/search`
7. **Workday** — JSON POST to `myworkdayjobs.com`
8. **RSS feeds** — WeWorkRemotely (Engineering + Management), HN Jobs, RemoteOK
9. **localfeed** — file-based adapter reading `data/feeds/{source}.json`; generated by `scripts/generate-feeds.mjs` (Playwright). Gracefully skips if files are missing or stale.
10. **Manual paste** — user provides URL directly
...
When applying external resume feedback (e.g., Ladders, resume coach), filter the feedback before acting:

- **Act on:** verb duplication, weak verbs, quantification gaps, over-long roles (>20 years old), structural AI-tells (colon scene-setters, em-dash clause separators)
- **Ignore:** ATS format warnings (already clean), bullet vs paragraph summary preference (paragraph is correct for Director-level), upsell language

Apply fixes to all three files in order: `resume.md` → `career-bank.md` → master HTML. Run verb audit after. Regenerate PDF last.

**Role compression rule:** Older roles (GD, etc.) may be compressed to 1 bullet to save space. Keep the most prestigious award; drop the rest. Never cut the role entirely.

**Master HTML vs resume.md divergence:** These files can accumulate different edits. When patching, always read the actual file content first — do not assume text matches. Write patch scripts to `/tmp/` and run via `cd <project-dir> && node /tmp/patch.mjs` (Node.js `readFileSync`/`writeFileSync` bypasses macOS file access restrictions that block direct shell reads on `~/Documents/`).
