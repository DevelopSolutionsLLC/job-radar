...
import { PATHS, loadPortals } from './lib/config.mjs';
...
function loadCompaniesFile() {
  ...
  if (existsSync(PATHS.companies)) {
    const lines = readFileSync(PATHS.companies, 'utf-8').split('\n');
    for (const line of lines) {
...
if (addTier && !dryRun) {
  ...
  if (toAdd.length === 0) {
    ...
    const resolveResults = await parallelLimit(
      toAdd.map(d => () => resolveAts(d.name).then(result => ({ d, result }))),
      RESOLVE_CONCURRENCY,
    );

    for (const { d, result } of resolveResults) {
    ...
    if (addedCount > 0) {
      writeFileSync(PATHS.portals, yaml.dump(config, { lineWidth: -1 }));
...
if (!dryRun && scored.length > 0) {
  ...
  writeFileSync(PATHS.companies, header + rows.join('\n') + '\n');
}
